...
class DvrStreamService(AbstractStreamService):
    ...
    _DVR_FILENAME_TPL = '{storage_path}/{cam_id}_{channel}_{segment_time}_%Y-%m-%d_%H-%M-%S.mp4'

    def __init__(self, *args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self._upload_engine = \
            DvrUploadEngine(conf=self.cam.conf.livestream.dvr, cam=self.cam)
    ...
    async def _start_upload_engine(self) -> None:
        ...
        for storage_settings in self._conf.upload.storage.values():
            if storage_settings.enabled \
                ...
                self._log.info('Starting Upload Engine for %s',
